...
export default function PuntoSwitcher(): Component{

    const [text, setText] = createSignal("");
    const [copyButtonContent, setCopyButtonContent] = createSignal("Copy!");
    const [output, setOutput] = createSignal("");

    const setCharAt = (string, index, char) => index > string.length -1 ? string : string.substr(0, index) + char + string.substr(index + 1)
    ...
    const changeLayout = (string: string): string => {
        if (string.length > 0) {
            for (let i = 0; i < string.length; i++) {
                ...
                if (rules.hasOwnProperty(key)) string = setCharAt(string, i, rules[key]);
